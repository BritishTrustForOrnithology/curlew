...
# Load biometric data
dt_biometric <- read.csv(file.path(datawd, "headstart_curlew_biometric_data_raw biometrics.csv"), header = TRUE, stringsAsFactors = FALSE) %>% dplyr::select(1:7, 9:11, 13:15, 17:21, 35:41)
...
dt_all <- merge(dt_biometric_ringing, dt_meta, by = c("flag_id", "year"))
