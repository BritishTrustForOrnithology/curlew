...
---
title: WWRG tagged curlew movements *by tide height* from `r params$set_title_1` to `r params$set_title_2`
---
...
```{r leaflet-map-by-tide-height, results='hide', eval = TRUE}

# convert lat lon to points
dt_map <- st_as_sf(tag_tide_merged_dt, coords = c("location.long","location.lat"), crs=4326)

## Create a continuous palette function
pal <- colorNumeric(
  palette = "YlGnBu",
  domain = dt_map$observed_m)
...
leaflet_map <- leaflet_map %>%
    addCircleMarkers(data = dt_map,
                     fill = TRUE,
                     radius = 1,
                     fillColor = ~pal(observed_m),
                     fillOpacity = 0.8,
                     stroke = TRUE,
                     color = ~pal(observed_m),
...
addLegend("bottomleft", pal = pal, values = dt_map$observed_m,
    title = "Observed tide height (m)",
    opacity = 1
  )
